...
{
    internal static class LineGeneratorUtils
    {
        // See wikipedia Bresenham's line algorithm
        public static void BresenhamLineGenerator(Point startPoint, Point endPoint, Action<int, int> plotAction)
        {
            if (plotAction == null)
                return;

            int x0 = (int)Math.Round(startPoint.X), x1 = (int)Math.Round(endPoint.X);
            int y0 = (int)Math.Round(startPoint.Y), y1 = (int)Math.Round(endPoint.Y);
        ...
        public static void XiaolinWuLineGenerator(Point startPoint, Point endPoint, Action<int, int, byte> plotAction)
        {
            if (plotAction == null)
                return;
